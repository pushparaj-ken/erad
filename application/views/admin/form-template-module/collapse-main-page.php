		...
		<div class="row">		
            <div class="col-md-12">
                <div class="box">
                    ...
                    <div class="box-body">
                        <div class="row">
                            <form class="insert_collapse_items_main" this_id="form-9900" reload-action="true">
								<div class="col-md-6">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
										...
										<select class="form-control select2" name="select_fields_id_sub[]" multiple="multiple" data-placeholder="Select a Hide Items" required style="width: 100%;">
											<?php foreach($all_inputs_sub as $fields) : ?>
								...
								<!--<div class="col-md-12">
                                    <div class="form-group">
										<label>Select Child Collapse Items</label>
										<select class="form-control select2" name="select_fields_id_child[]" multiple="multiple" data-placeholder="Select a Hide Items" required style="width: 100%;">
											<?php foreach($all_inputs_child as $fields) : ?>
												<option value="<?= $fields->id ?>"><?= $fields->name?></option>
											<?php endforeach; ?>
										</select>
									</div>
						...
						<table class="table table-hover data_table">
							<thead>
								<tr>
									...
									<th>Item Name</th>
									<th>Title Name</th>
									<th>Label Name Sub </th>
									<!--<th>Label Name Child </th>-->
							...
							<tbody>
								...
								?>
										<tr>
											...
											<td>
												<?php foreach(explode("," , $record->select_fields_id_sub) as $collapse):?>
													<?= $this->common_model->get_record('tbl_form_template_fields', "status='0' and id='$collapse'", "name") ?>,
												<?php endforeach;?>
											</td>
											<!--<td>
												<?php foreach(explode("," , $record->select_fields_id_child) as $collapse):?>
													<?= $this->common_model->get_record('tbl_form_template_fields', "status='0' and id='$collapse'", "name") ?>,
												<?php endforeach;?>
											</td>-->
											...
											<td class="text-center" style="display: flex;justify-content: center">
												<span class="btn btn-sm btn-warning" onclick="edit_collapse_main(<?= $record->id ?>,<?= $record->template_id ?>,<?= $record->input_id ?>);">
													<i class="fa fa-edit"></i>
												</span>
												<form class="update_data" this_id="form-<?= uniqid() ?>" reload-action="true">
                                                    <input type="hidden" name="row_id" value="<?= $record->id ?>">
                                                    <input type="hidden" name="status" value="1">
                                                    <input type="hidden" name="table_name" value="tbl_collapse_section_main">
...
<div class="modal fade" id="modal-default">
	<div class="modal-dialog">
		<div class="modal-content">
			<form reload-action="true" this_id="form-002" class="update_collapse_items_main" method="post" role="form">
...
<script>
function edit_collapse_main(row_id,template_id,input_id)
{
	$.ajax({
		type: 'POST',
		url: baseURL + "admin/edit_collapse_main",
...
$('body').on("submit", '.insert_collapse_items_main', function(e) {
    e.preventDefault();
    ...
    if (is_required(this_id) === true) {
        $.ajax({
            type: 'POST',
            url: baseURL + "admin/insert_collapse_items_main",
...
$('body').on("submit", '.update_collapse_items_main', function(e) {
    e.preventDefault();
    ...
    if (is_required(this_id) === true) {
        $.ajax({
            type: 'POST',
            url: baseURL + "admin/update_collapse_items_main",
            data: new FormData(this),
            dataType: "json",
            contentType: false,
            processData: false,
