						...
						<div class="row">
							<form class="insert_data" this_id="form-9900" reload-action="true">
								...
								<div class="col-md-6">
									...
									<input type="hidden" name="table_name" value="tbl_instant_click">
							...
							<tbody>
								...
								?>
										<tr>
											...
											<td class="text-center">
												<?= ($record->condition_id == 1) ? ('BEGINNING') : ('') ?>
												<?= ($record->condition_id == 2) ? ('MIDDLE') : ('') ?>
												<?= ($record->condition_id == 3) ? ('END') : ('') ?>
											</td>
											<td>
												<?= $record->name ?>
											</td>
											...
											<td class="text-center" style="display: flex;justify-content: center">
												<a class="btn btn-sm btn-warning" style="margin-right: 5px;" data-toggle="modal" data-target="#modal-default" onclick="$('.update_data input[name=row_id]').val(`<?= $record->id ?>`);$('.update_data input[name=name]').val(`<?= $record->name ?>`);$('.update_data select[name=condition_id]').val(`<?= $record->condition_id ?>`);$('.update_data select[name=template_id]').val(`<?= $record->template_id ?>`);"><i class="fa fa-pencil-square-o"></i></a>
